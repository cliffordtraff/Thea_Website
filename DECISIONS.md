...
### D3. Placeholder imagery — labeled neutral SVG blocks
- **Decision:** All photographs are replaced by flat-tone SVG placeholder blocks,
  each labeled with its layout role and aspect ratio, generated by
  `scripts/gen-placeholders.mjs`. Real photos are never scraped.
- **Why:** Content rule — do not scrape/republish the site's photographs. SVG
  keeps files tiny, exact aspect ratios, and adds no runtime dependency.
- **Note:** `next.config` sets `images.dangerouslyAllowSVG: true` **only** because
  every SVG is first-party and generated by us (no user input). This is a POC
  convenience, documented here.
- **Reverse:** Drop real `.jpg`/`.webp` assets into `public/`, update the `src`
  and true `width`/`height` in the `content/` data, remove the SVG allow flag.
